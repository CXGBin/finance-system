...
public class ExpenseClaimController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ApiResult<long>> Create([FromBody] ExpenseClaimRequest request) => ApiResult<long>.Success(await _service.CreateAsync(request, 0));

    [HttpPost("{id}/submit")]
    public async Task<ApiResult<bool>> Submit(long id) { await _service.SubmitAsync(id, 0); return ApiResult<bool>.Success(true); }
