...
import { Card, Descriptions, Button, Statistic, Row, Col, message } from 'antd';
...
const AssetCardDetail: React.FC = () => {
  ...
  const loadData = async (vid: number) => {
    setLoading(true);
    try { const res = await assetApi.cardDetail(vid); setData(res.data || null); } finally { setLoading(false); }
  };

  return (
    <Card title="资产详情" loading={loading} extra={<Button onClick={() => navigate(-1)}>返回</Button>}>
      {data && (<>
